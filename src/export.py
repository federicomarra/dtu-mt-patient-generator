    ...
    for p_id, p_data in results_dict.items():
        ...
        for day, values in days.items():
            ...
            if isinstance(values, Mapping):
                ...
                cho_announced_arr = np.full(values_arr.size, 0.0, dtype=np.float64)
                scenario_id = None
                missed_meal_id = None
                late_bolus_id = None
                late_bolus_ids_val = []
            ...
            if cho_announced_arr.size != values_arr.size:
                cho_ann_aligned = np.full(values_arr.size, 0.0, dtype=np.float64)
                common = min(cho_announced_arr.size, values_arr.size)
                if common > 0:
            ...
            blocks.append(pd.DataFrame({
                ...
                "cho_mg_announced": cho_announced_arr.astype(float),
